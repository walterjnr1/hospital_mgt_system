...
$hiddenId = $_POST['hidden_id'];
 ...
 if($displayName !='' && $userName !='' && $password !='' && $status !='') {
  ...
  '$password' , `profile_picture` = '$targetFile' , `sex` = '$sex' ,`phone` = '$phone' , `email` = '$email' where `id` = $hiddenId";

}elseif ($displayName !=='' && $userName !=='' && $password !==''){

  $updateUserQuery = "UPDATE `users` set `display_name` = '$displayName' ,`user_name` = '$userName' , `sex` = '$sex',`phone` = '$phone'  , `email` = '$email',`password` = 
  '$password' 
  where `id` = $hiddenId";

}elseif ($displayName !=='' && $userName !=='' && $status !==''){

  $updateUserQuery = "UPDATE `users` set `display_name` = '$displayName' ,`phone` = '$phone' , `user_name` = '$userName' , `profile_picture` = '$targetFile ' 
   where `id` = $hiddenId";
}
else {
...
<head>
 ...
 <title>Update User  Details - DESIGN AND DEVELOPMENT OF A WEB BASED HOSPITAL MANAGEMENT SYSTEM FOR ARTHUR JARVIS UNIVERSITY</title>
            ...
            <form method="post" enctype="multipart/form-data">
              ...
              <div class="row">
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-10">
                  ...
                  class="form-control form-control-sm rounded-0" value="<?php echo $row['sex'];?>"/>

                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-10">
                  <label>Password</label> 
                  <input type="password" id="password" name="password" 
                  class="form-control form-control-sm rounded-0" value="<?php echo $row['password'];?>"/>

                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-10">
                  <label>Email</label> 
                  <input type="email" id="email" name="email" 
                  class="form-control form-control-sm rounded-0" value="<?php echo $row['email'];?>"/>

                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-10">
                  <label>Phone</label> 
                  <input type="tel" id="phone" name="phone" 
                  class="form-control form-control-sm rounded-0" value="<?php echo $row['phone'];?>"/>
