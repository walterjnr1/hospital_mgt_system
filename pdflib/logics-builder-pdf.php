...
class LB_PDF extends FPDF {
    ...
    public function Header() {
        if ($this->pageHeaderRepeat || (!$this->pageHeaderRepeat && !$this->pageHeaderAdded)) {
            // Logo
            $this->Image('dist/img/logo.png', $this->GetX(), $this->GetY(), 70);
            // Times bold 13
            $this->SetFont('', 'B', self::HEADING_SIZE);
            // Title
            $this->Cell(0, 7, 'DESIGN AND DEVELOPMENT OF A WEB BASED HOSPITAL MANAGEMENT SYSTEM FOR ARTHUR JARVIS UNIVERSITY ', 0, 0, 'C');
            $this->Ln();
